...
(function() {
  ...
  Query = (function() {
    ...
    Query.prototype._sortFn = function(input, cb) {
      var output;
      this._debug("_sortFn(input, cb?)" + " // sort options by probability".grey);
    ...
    Query.prototype._toAllFn = function(args, cb) {
      ...
      __toAllFn = (function(_this) {
        return function(args) {
          ...
          _this._debug("_toAllFn:__toAllFn(" + (pretty(args)) + ")");
          res = {};
          _ref = args.result;
          _results = [];
          for (key in _ref) {
            ...
            _results.push(res[key] = (function() {
              ...
              if (args.types[key] === 'Number') {
                this._debug("_toAllFn:__toAllFn: Number -> " + key);
                _results1 = [];
                for (option in options) {
                  weight = options[option];
                ...
                this._debug("_toAllFn:__toAllFn: Boolean -> " + key);
                _results2 = [];
                for (option in options) {
                  weight = options[option];
                ...
                this._debug("_toAllFn:__toAllFn: String -> " + key);
                _results3 = [];
                for (option in options) {
                  weight = options[option];
    ...
    Query.prototype._sync = function() {
      ...
      results = (function() {
        switch (this._mode) {
          case 'all':
            this._debug("_sync: all: @_toAllFn(" + (pretty(ctx)) + ")");
            results = this._toAllFn(ctx);
          ...
          switch (_this._mode) {
            case 'all':
              ...
              return _this._toAllFn(ctx, function(results) {
                _this._debug("_async: all: @_sortFn(results, cb)");
